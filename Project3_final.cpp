...
struct DefinedFunctionInfo{
  ...
  map< string, DefinedIDInfo > localIDTable ;
} ;
...
class Parser {
  public:
  Token mBuffer ;
  map< string, DefinedFunctionInfo > mFunctionTable ;
  map< string, DefinedFunctionInfo >::iterator mIterF ;
...
vector<Token> InfixToPostfix( vector<Token> mTokenVec ) {
  ...
  for ( int i = 0 ; i < mTokenVec.size() ; i++ ) {
    ...
    if ( mTokenVec[i].type == CONSTANT || mTokenVec[i].type == IDENT ) {
...
void Execute_Statement( vector<Token> tokenList ) {
  int isCout = 0 ;
...
bool Parser::FindDefinedFunction( string name ) {  
  // , float &value 
  mIterF = mFunctionTable.find( name ) ;      // 找ID是否被defined
  if ( mIterF == mFunctionTable.end() )       // 找不到
    return false;
  else          
    return true;
...
void Parser::DefineGlobalFunction() {    // ex. void f(int &a){int b,c[10];}
  ...
  tempDefinedFunction.content = new vector< string >() ;
  string tempFunctionName = "\0", space = "\0", tempContent = "\0" ;
  
  tempFunctionName = mTokenList[1].name + "()" ;     // 儲存function名稱
  mIterF = mFunctionTable.find( mTokenList[1].name ) ;   // 判斷是否被定義過
  ...
  if ( mIterF == mFunctionTable.end() ) {
    mFunctionTable[mTokenList[1].name] = tempDefinedFunction ;
...
bool Parser::SystemFunctions() {
  if ( mTokenList[0].name == "ListAllVariables" ) {
    ...
    for ( mIterF = mFunctionTable.begin() ; mIterF != mFunctionTable.end() ; mIterF++ )
      if ( gIterID->first != "cout" )
        cout << mIterF->first << "()" << endl;
  } // else if
    ...
    if ( FindDefinedFunction( mTokenList[2].name.substr( 1, mTokenList[2].name.size()-2 ) ) ) {     // 字串去掉""
      ...
      tempFunc.content = new vector< string >() ;
      tempFunc = mFunctionTable[ mTokenList[2].name.substr( 1, mTokenList[2].name.size()-2 ) ] ;
      for ( int i = 0 ; i < tempFunc.content->size() ; i++ ) {
        cout << tempFunc.content->at( i ) << endl ;
      } // for
    } // if
  } // else if
  else
